...
/* VSOMEIP_INFO << __func__ << ": multicast_ttl_:" << multicast_ttl_;
    ...
    this->max_message_size_ = configuration_->get_max_message_size_reliable(
            _local.address().to_string(), _local.port());
    this->queue_limit_ =
            configuration_->get_endpoint_queue_limit(_local.address().to_string(), _local.port());
}
...
void tcp_server_endpoint_impl::start() {
    ...
    if (acceptor_.is_open()) {
        connection::ptr new_connection = connection::create(
                ...
                max_message_size_, buffer_shrink_threshold_, has_enabled_magic_cookies_, io_,
                send_timeout_);
...
bool tcp_server_endpoint_impl::send_to(const std::shared_ptr<endpoint_definition> _target,
...
bool tcp_server_endpoint_impl::send_queued(const target_data_iterator_type _it) {
    ...
    connection::ptr its_connection;
    {
        ...
        if (connection_iterator != connections_.end()) {
            ...
            VSOMEIP_INFO << "Didn't find connection: " << _it->first.address().to_string() << ":"
                         << std::dec << static_cast<std::uint16_t>(_it->first.port())
                ...
                for (const auto& its_q : _it->second.queue_) {
                    ...
                    if (its_buffer && its_buffer->size() > VSOMEIP_SESSION_POS_MAX) {
                        service_t its_service =
                                bithelper::read_uint16_be(&(*its_buffer)[VSOMEIP_SERVICE_POS_MIN]);
...
void tcp_server_endpoint_impl::get_configured_times_from_endpoint(
    ...
    configuration_->get_configured_timing_responses(
            _service, tcp_server_endpoint_base_impl::local_.address().to_string(),
...
void tcp_server_endpoint_impl::accept_cbk(connection::ptr _connection,
    ...
    if (_error != boost::asio::error::bad_descriptor
        ...
        VSOMEIP_ERROR << "tcp_server_endpoint_impl::accept_cbk: " << _error.message() << " ("
                      << std::dec << _error.value() << ") Will try to accept again in 1000ms";
        auto its_timer =
                std::make_shared<boost::asio::steady_timer>(io_, std::chrono::milliseconds(1000));
...
tcp_server_endpoint_impl::connection::connection(
        const std::weak_ptr<tcp_server_endpoint_impl>& _server, std::uint32_t _max_message_size,
    ...
    socket_(_io), server_(_server), max_message_size_(_max_message_size),
    recv_buffer_size_initial_(_recv_buffer_size_initial),
    recv_buffer_(_recv_buffer_size_initial, 0), recv_buffer_size_(0), missing_capacity_(0),
    shrink_count_(0), buffer_shrink_threshold_(_buffer_shrink_threshold), remote_port_(0),
...
tcp_server_endpoint_impl::connection::~connection() {
    ...
    if (its_server) {
        ...
        if (its_routing_host) {
            its_routing_host->remove_subscriptions(its_server->local_port_, remote_address_,
                                                   remote_port_);
        }
    }
}
...
tcp_server_endpoint_impl::connection::ptr tcp_server_endpoint_impl::connection::create(
    ...
    return ptr(new connection(_server, _max_message_size, its_initial_receveive_buffer_size,
                              _buffer_shrink_threshold, _magic_cookies_enabled, _io,
                              _send_timeout));
}
...
void tcp_server_endpoint_impl::connection::receive() {
    ...
    if (socket_.is_open()) {
            ...
            if (missing_capacity_) {
                ...
                if (its_capacity < its_required_capacity) {
                    ...
                    if (recv_buffer_.size() > 1048576) {
                        VSOMEIP_INFO << "tse: recv_buffer size is: " << recv_buffer_.size()
                ...
                missing_capacity_ = 0;
            } else if (buffer_shrink_threshold_ && shrink_count_ > buffer_shrink_threshold_
                       && recv_buffer_size_ == 0) {
        ...
        socket_.async_receive(
                ...
                std::bind(&tcp_server_endpoint_impl::connection::receive_cbk, shared_from_this(),
...
void tcp_server_endpoint_impl::connection::send_queued(const target_data_iterator_type _it) {
    ...
    const session_t its_session =
            bithelper::read_uint16_be(&(*its_buffer)[VSOMEIP_SESSION_POS_MIN]);
    if (magic_cookies_enabled_) {
        const std::chrono::steady_clock::time_point now = std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::milliseconds>(now - last_cookie_sent_)
...

    {
        ...
        boost::asio::async_write(
                ...
                std::bind(&tcp_server_endpoint_impl::connection::write_completion_condition,
                          ...
                          its_buffer->size(), its_service, its_method, its_client, its_session,
                          std::chrono::steady_clock::now()),
                std::bind(&tcp_server_endpoint_base_impl::send_cbk, its_server, _it->first,
                          std::placeholders::_1, std::placeholders::_2));
    }
}
...
void tcp_server_endpoint_impl::connection::receive_cbk(boost::system::error_code const& _error,
                                                       std::size_t _bytes) {
            ...
            do {
                ...
                if (has_full_message) {
                    ...
                    if (is_magic_cookie(its_iteration_gap)) {
                        ...
                        if (magic_cookies_enabled_) {
                            uint32_t its_offset = its_server->find_magic_cookie(
                                    &recv_buffer_[its_iteration_gap], recv_buffer_size_);
                                ...
                                if (!is_magic_cookie(its_iteration_gap)) {
                                    ...
                                    if (its_endpoint_host) {
                                        its_endpoint_host->on_error(
                                                ...
                                                static_cast<length_t>(recv_buffer_size_),
                    ...
                    if (needs_forwarding) {
                        if (utility::is_request(
                            ...
                            if (its_client != MAGIC_COOKIE_CLIENT) {
                                const service_t its_service = bithelper::read_uint16_be(
                                        &recv_buffer_[its_iteration_gap + VSOMEIP_SERVICE_POS_MIN]);
                                const method_t its_method = bithelper::read_uint16_be(
                                ...
                                its_server->clients_[to_clients_key(its_service, its_method,
                                                                    its_client)][its_session] =
                                        remote_;
                            }
                        }
                        if (!magic_cookies_enabled_) {
                            its_host->on_message(&recv_buffer_[its_iteration_gap],
                                                 current_message_size, its_server.get(), false,
                                                 VSOMEIP_ROUTING_CLIENT, nullptr, remote_address_,
                                                 remote_port_);
                        } else {
                ...
                if (!has_full_message) {
                    if (recv_buffer_size_ > VSOMEIP_RETURN_CODE_POS
                        && (recv_buffer_[its_iteration_gap + VSOMEIP_PROTOCOL_VERSION_POS]
                                    != VSOMEIP_PROTOCOL_VERSION
                            || !utility::is_valid_message_type(static_cast<message_type_e>(
                                    recv_buffer_[its_iteration_gap + VSOMEIP_MESSAGE_TYPE_POS]))
                            || !utility::is_valid_return_code(static_cast<return_code_e>(
                                    recv_buffer_[its_iteration_gap + VSOMEIP_RETURN_CODE_POS])))) {
                        if (recv_buffer_[its_iteration_gap + VSOMEIP_PROTOCOL_VERSION_POS]
                            != VSOMEIP_PROTOCOL_VERSION) {
                            {
                                std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            ...
                            its_host->on_message(&recv_buffer_[its_iteration_gap],
                                                 ...
                                                 false, VSOMEIP_ROUTING_CLIENT, nullptr,
                                                 remote_address_, remote_port_);
                        ...
                        if (magic_cookies_enabled_) {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR
                                    << "Received a TCP message which exceeds "
                        ...
                        } else {
                            {
                                ...
                                VSOMEIP_ERROR << "Received a TCP message which exceeds "
                                              << "maximum message size (" << std::dec
                                              << current_message_size << " > " << std::dec
                                              << max_message_size_
                                              << ") Magic cookies are disabled: "
                                              << "Connection will be closed! local: "
                                              << get_address_port_local()
                                              << " remote: " << get_address_port_remote();
                            }
                            wait_until_sent(boost::asio::error::operation_aborted);
                            return;
...
std::size_t tcp_server_endpoint_impl::connection::write_completion_condition(
...
void tcp_server_endpoint_impl::print_status() {
    ...
    VSOMEIP_INFO << "status tse: " << std::dec << local_port_ << " connections: " << std::dec
    ...
    for (const auto& c : its_connections) {
        ...
        VSOMEIP_INFO << "status tse: client: " << c.second->get_address_port_remote()
                     << " queue: " << std::dec << its_queue_size << " data: " << std::dec
                     << its_data_size << " recv_buffer: " << std::dec << its_recv_size;
    }
}
